...
public class StatisticsController {
    ...
    public ResponseVO<List<Map<String, Object>>> getScenicSalesTop10() {
        ...
        String sql = "SELECT " +
                ...
                "FROM scenic_info s " +
                "LEFT JOIN order_info o ON s.id = o.scenic_id AND o.status != 'CANCELLED' " +
    ...
    public ResponseVO<List<Map<String, Object>>> getCategoryStatistics() {
        ...
        String sql = "SELECT " +
                ...
                "COALESCE(COUNT(CASE WHEN o.status != 'CANCELLED' THEN o.id END), 0) AS orderCount " +
    ...
    public ResponseVO<Map<String, Object>> getOverview() {
        ...
        // 订单总数（不包括已取消）
        String orderCountSql = "SELECT COUNT(*) FROM order_info WHERE status != 'CANCELLED'";
        ...
        // 总销售额（按实际订单金额计算）
        String totalRevenueSql = "SELECT COALESCE(SUM(total_amount), 0) FROM order_info WHERE status != 'CANCELLED'";
