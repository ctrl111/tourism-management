  ...
  <div>
    <el-space direction="vertical" alignment="left" style="width: 100%">
      ...
      <el-card>
        <el-table ref="tableComponents"
          ...
          <el-table-column prop="attractionId" label="景点id"></el-table-column>
    ...
    >
      <el-form ref="formRef" :model="formData" label-width="100px">
        <slot name="content">
          <el-form-item label="订单id" prop="orderId"  :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
          ...
          <el-form-item label="购买数量" prop="quantity"  :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
            <el-input v-model="formData.quantity"></el-input>
          </el-form-item>
          <el-form-item label="price" prop="price"  :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
...
function submit() {
  formRef.value.validate((valid) => {
    ...
    if (!formData.value.id) {
      request.post("/orderItem/add", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
      ...
      //更新
      request.put("/orderItem/update", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
...
function batchDelete(rows) {
  ...
  ElMessageBox.confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', {
    ...
    request.delete("/orderItem/delBatch", {data: ids}).then(res => {
      if (!res) {
        return
      }
      ElMessage({
