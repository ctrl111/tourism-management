    ...
    <el-space direction="vertical" alignment="left" style="width: 100%">
      <el-card>
        <el-form ref="searchFormComponents" :model="searchForm" inline>
          <el-form-item label="用户" prop="categoryType">
            <el-input v-model="searchForm.username" clearable></el-input>
          </el-form-item>
          <el-form-item label="景点名称" prop="name">
            <el-input v-model="searchForm.scenicName" clearable></el-input>
          </el-form-item>
          <el-form-item label="">
      ...
      <el-card>
        <el-table ref="tableComponents"
          ...
          <el-table-column prop="scenicName" label="景点"></el-table-column>
    ...
    >
      <el-form ref="formRef" :model="formData" label-width="100px">
        <slot name="content">
          ...
          <el-form-item label="评分" prop="score"  :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
...
function submit() {
  formRef.value.validate((valid) => {
    ...
    if (!formData.value.id) {
      request.post("/scenicComment/add", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
      ...
      //更新
      request.put("/scenicComment/update", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
...
function batchDelete(rows) {
  ...
  ElMessageBox.confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', {
    ...
    request.delete("/scenicComment/delBatch", {data: ids}).then(res => {
      if (!res) {
        return
      }
      ElMessage({
